...
class Canvas {
    constructor(width = window.innerWidth, height = window.innerHeight) {
        ...
        this.createCanvas();
        this.createGenesisShapes();
        this.drawShapes();

        setInterval(() => {
            this.createNextShapes();
    ...
    createGenesisShapes() {
        for (let i = 0; i < getRandomInt(this.UPPER_BLOCK_LIMIT); i++) {
            this.rects[i] = {
                ...
                height:
                    getRandomInt(this.height) / this.UPPER_BLOCK_SIZE_LIMIT_DIVISION,
            };
        }
    }
        ...
        for (let i = 0; i < this.rects.length; i++) {
            this.rects[i] = {
                ...
                x: getRandomIntInRange(
                    this.rects[i].x - this.nextBidirectionalRange,
                    this.rects[i].x + this.nextBidirectionalRange
                ),
                y: getRandomIntInRange(
                    this.rects[i].y - this.nextBidirectionalRange,
                    this.rects[i].y + this.nextBidirectionalRange
                ),
